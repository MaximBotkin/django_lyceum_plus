...
class RegistrationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if CustomUser.objects.filter(email=email):
            self.add_error(
                CustomUser.email.field.name, 'Пользователь с таким email уже существует, попробуйте войти в аккаунт.'
            )
        if CustomUser.objects.filter(username=username):
            self.add_error(
                CustomUser.username.field.name, 'Пользователь с таким ником уже существует, придумайте новый никнэйм.'
            )
        return cleaned_data
